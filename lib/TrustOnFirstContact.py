...
class TrustOnFirstContact(ACNode):
  ...
  def setup(self):
    ...
    if self.cnf.newprivatekey:
       ...
       self.cnf.publickey == curve.generatePublicKey(private_key_master)
  ...
  def announce(self,dstnode):
    if self.cnf.privatekey:
    	return self.send(dstnode, "announce " + socket.gethostbyname(socket.gethostname()) + " " + base64.b64encode(self.cnf.privatekey)
