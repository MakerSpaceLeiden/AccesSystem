...
void SIG2::loop() {
    ...
    if (init_done == 1) {
#if 0
        // We're printing the version of mbedtls - as there has been some fiddling with the ECP api.
        char tmp[20];
        mbedtls_version_get_string_full(tmp);
        Debug.printf("Generating Curve25519 session keypair with %s\n",tmp);
#else
        Debug.printf("Generating Curve25519 session keypair\n");
#endif
        resetWatchdog();
