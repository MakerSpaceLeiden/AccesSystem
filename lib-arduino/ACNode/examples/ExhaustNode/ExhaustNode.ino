...
SyslogStream SyslogStream(); // default syslog port and destination (gateway address).
MqttLogStream MqttLogStream("log");
...
void setup() {
  ...
  node.addLogger(Serial, ACLog::DEBUG);
  // node.addLogger(SyslogStream, ACLog::VERBOSE);
  // node.addLogger(MqttStream, ACLog::WARN);
