...
void reportUse(bool isFinal) {
  if (isFinal)
    Log.printf("Final %.1f; by %s", (whCounter - startWhCounter) / 1000., "xxx");
  else if (machinestate == POWERED)
    Log.printf("Use %.1f, kWh Counter: %.1f", (whCounter - startWhCounter) / 1000., whCounter / 1000.);
  else
    Log.printf("idle, kWh Counter: %.1f", whCounter / 1000.);
}
