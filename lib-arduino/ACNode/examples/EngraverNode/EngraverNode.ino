...
#define SOLENOID_GPIO (node.OUT1)   // Bottom relay; wired to switch 24volt
#define SENSE_PIN     (node.OPTO1)  // Low when locked, High when open

#define BUZZ_TIME     (250) // How long to engage, in milli seconds

#define LED_GREEN   (node.LEDD)
#define LED_RED  (node.LEDE)
...
void loop() {
  ...
  //
  digitalWrite(LED_GREEN, digitalRead(SENSE_PIN) == LOW); // Green if lock is in the locked position
  digitalWrite(LED_RED, digitalRead(SENSE_PIN) == HIGH);  // Red if the lock/door is opened
